	...
	if($maxacc > 0){
		for ($k=0; $k < $maxacc; $k++){
			for ($j=0; $j < 2; $j++){
				...
				if(stristr($data, "window.location.href='/file/") || stristr($data, "hash = hash[h]")) {
					$url = $this->curl('http://vinaget.us/gator/','','data='.urlencode($data),0);
					$url = trim($url);
					$data = $this->curl($url,$cookie.';lang=en',"");
				}
				...
				if(isset($link) && stristr($link, 'http')) {
					...
					if($size_name[0] > 200 ){
					//	$filesize =  round($size_name[0]/(1024*1024*1024),2)." TB";
					//	$filesize =  round($size_name[0]/(1024*1024),2)." GB";
					//	$filesize =  round($size_name[0]/(1024),2)." GB";
