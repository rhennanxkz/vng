...
if (preg_match('#^http://(www\.)?ryushare\.com/#', $url)){
	...
	if(empty($cookie)==false || ($user && $pass)){
		for ($j=0; $j < 2; $j++){
			...
			elseif (stristr($data,'<b>File Not Found</b>')) die(Tools_get::report($Original,"dead"));
			elseif(preg_match_all('/input type="hidden" name="(.*?)" value="(.*?)"/i', $data1, $value)) {
				...
				if(preg_match('%a href="(http://[a-z].+\.ryushare.com/files/.*)">Click here to download%U', $data, $matches)) {
