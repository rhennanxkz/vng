...
/*
@h@	PRE-RELEASE DEBUG PLUGIN - DO NOT LEAK 
*/
...
if (empty($_REQUEST['action']) || $_REQUEST['action'] != 'Th3-822') {
	...
	if (!empty($_REQUEST['T8']['up_login']) && !empty($_REQUEST['T8']['up_pass'])) SavedLogin($_REQUEST['T8']['up_login'], $_REQUEST['T8']['up_pass']);
		...
		echo "<b><center>Login not found or empty, using ephemeral user upload.</center></b>\n";
		$T8['master_key'] = $password_aes = $session_self_challenge = array();
		for ($x = 0; $x < 4; $x++) {
			$T8['master_key'][] = get_rand(4);
			$password_aes[] = get_rand(4);
			$session_self_challenge[] = get_rand(4);
		}

		$uh = apiReq(array('a' => 'up', 'k' => a32_to_base64(encrypt_key($T8['master_key'], $password_aes)), 'ts' => base64url_encode(a32_to_str($session_self_challenge) . a32_to_str(encrypt_key($session_self_challenge, $T8['master_key'])))));
		if (is_numeric($uh[0])) check_errors($uh[0], 'Cannot check ephemeral user handle');

		//echo "<b><center>Ephemeral user handle: \"".htmlentities($uh[0])."\"</center></b>\n"; // Why show this?
		$res = apiReq(array('a' => 'us', 'user' => $uh[0]));
		if (is_numeric($res[0])) check_errors($res[0], 'Error while getting ephemeral user master key');

		if (empty($res[0]['tsid'])) html_error('Ephemeral user session id not found.');
		$T8['master_key'] = decrypt_key(base64_to_a32($res[0]['k']), $password_aes);
		$tsid = base64url_decode($res[0]['tsid']);
		if (a32_to_str(encrypt_key(str_to_a32(substr($tsid, 0, 16)), $T8['master_key'])) != substr($tsid, -16)) html_error('Ephemeral user session id is invalid.');
		$T8['sid'] = $res[0]['tsid'];
		getRootNode();
		unset($password_aes, $session_self_challenge, $uh, $res, $tsid); // We don't need them now
	}
...
function check_errors($err, $prefix = 'Error') {
	switch ($err) {
		...
		case -9: $msg = 'Resource not found';break;
...
function SavedLogin($user, $pass) {
	...
	if (!file_exists($filename) || filesize($filename) <= 6) return MegaLogin($user, $pass);
	...
	if (is_array($savedcookies) && array_key_exists($hash, $savedcookies)) {
		...
		if ((is_array($cookie) && count($cookie) < 1) || empty($cookie)) return MegaLogin($user, $pass);
