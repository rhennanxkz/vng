...
if(empty($cookie)==false || ($user && $pass)){
	for ($j=0; $j < 2; $j++){
		...
		if(!$cookie){
			$data = $this->curl("http://www.multi-debrid.com","","");	
			$cookie = $this->GetCookies($data);
			$data = $this->curl("http://www.multi-debrid.com/login",$cookie,"user%5Bidentity%5D=talama95&user%5Bpass%5D=23041995&action=login");	
		...
		$data = $this->curl('http://www.multi-debrid.com/ajaxdownloader?link='.urlencode($url),$cookie,"",0);
