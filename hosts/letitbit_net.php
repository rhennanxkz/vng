	...
	if(empty($cookie)==false || ($user && $pass)){
		for ($j=0; $j < 2; $j++){
			...
			if (stristr($cookie,'prekey')) {
				...
				$this->cookie = $cookies;
				if (stristr($data,'direct_link_2')) $data = trim ($this->cut_str ($data, "direct_link_1", "direct_link_2" ));
				else $data = trim ($this->cut_str ($data, "var direct_links", "direct_link_1" ));
				if (preg_match('%(http:\/\/.+)" :%U', $data, $value)) {
					$link = trim($value[1]);
				...
				$this->cookie = $cookie.';'.$this->GetCookies($data);
				if (stristr($data,'File not found</p>')) die('<font color=red>Download Blocked (ip).</font>');

				$schema = parse_url($check2);
				if(preg_match ( '/ocation: \/sms\/check2.php/', $data, $linkpre)) {
					$check3 = $schema['scheme'].'://'.$schema['host'].'/sms/check2.php';
				}
				else {
					$cookie = "";
					$this->save_cookies("letitbit.net","");
...
* Version: 2.6.3
* Created: ..:: [H] ::..
* Updated: fixed by [H] (Sunday, 16 February 2014)
*/
?>