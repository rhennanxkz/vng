...
function showOrHide(){
	if ($('#showacc').css('display') == "none") {
		...
		$("#moreacc").html(less_acc);
	}
	else {
		$('#showacc').slideUp(800);
		$("#moreacc").html(more_acc);
	}
}
...
function get(obj) {
	var down = $("#submit").val();
	$('#submit').attr('disabled', 'disabled').val("Waiting...!");
	setTimeout(function() {
		$('#submit').removeAttr('disabled').val(down);
	},2000);
	$("#bbcode").html("");
...
function ajaxget(id, url, type){
    var autopcbox = $("input[name='autopcbox']:checked").val();
	var nick = $("#nick").val();
	var pass = $("#pass").val();
	...
	if(i > 4 && type !== 'reget') time = (Math.round(i/5)*3000);
	else time = 0;
	setTimeout(function(){
		$.ajax({
			type: "POST",
			url: "index.php?rand="+ Math.random(),
			data: "urllist="+encodeURIComponent(url)+'&captcha='+captcha+"&autopcbox="+autopcbox+"&nick="+encodeURIComponent(nick)+"&pass="+encodeURIComponent(pass)+"&"+param,
			success: function(html) {
				/* limit ip for mod  */
				if (errorlogin === true) return;
				if (/errorlogin/.test(html) === true) {
					errorlogin = true;
					alert("Ph&aacute;t hi?n c&oacute; s? thay ??i IP. Vui l&ograve;ng ??ng nh?p l?i!");
				...
				if (/type=\"password\" name=\"secure\"/i.test(html) === true) {
					...
					if(/shortencode/.test(html) === true) {
						var code = /shortencode \'(.*?)\'/g;
						var shortencode = code.exec(html);
						var bosung = '<input type="hidden" name="shortencode" value="'+shortencode[1]+'" />';
					}
					else var bosung = "";
					$("#link"+id).html('<center><form action="javascript:ajaxget(\''+id+'\',\''+url+'\',\'reget\');" name="formlink" id="formlink"><b><img src="http://www.google.com/recaptcha/api/image?c='+captcha[1]+'"/><br><br>Authentication Required<br><input type="text" name="recaptcha_response_field" value="" size="20" maxlength="50" /><input type="hidden" name="recaptcha_challenge_field" value="'+captcha[1]+'" />'+bosung+'<input type="submit" value="Go"/> <a onclick="ajaxget(\''+id+'\',\''+url+'\',\'refresh\');" href="javascript:void(0)" style="TEXT-DECORATION: none"><font color=#FF6600>Refresh</font></a></b></form></center>');
				}
				else if(/cookie code/.test(html) === true) {
					var text = /cookie code \'(.*?)\'/g;
					var captcha = text.exec(html);
					$("#link"+id).html('<center><form action="javascript:ajaxget(\''+id+'\',\''+url+'\',\'reget\');" name="formlink" id="formlink"><b><img src="captcha.png"/><br><br>Authentication Required<br><input type="text" name="captch" value="" size="20" maxlength="50" /><input type="hidden" name="cooki" value="'+captcha[1]+'" /><input type="submit" value="Go"/> <a onclick="ajaxget(\''+id+'\',\''+url+'\',\'refresh\');" href="javascript:void(0)" style="TEXT-DECORATION: none"><font color=#FF6600>Refresh</font></a></b></form></center>');
				}
				else if(/is this link sex/.test(html) === true) {
					$("#link"+id).html('<b><font color="red">Is this link sex ??? ==&#9658; </font><a target="'+id+'" style=\'TEXT-DECORATION: none\' href="http://www.google.com.vn/search?q='+url+'"><font color="#0066FF">'+url+'</font></a> <a onclick="return ajaxget(\''+id+'\',\''+url+'|not3x\',\'reget\');" href="javascript:void(0)" style=\'TEXT-DECORATION: none\'><font color=#ffcc33>&nbsp; If not click here to try again</font></a></b>');
				}
...
function checkacc(type){
	if (type.length > 0){
		if(type == "all"){
			$("[id^='unknown']").html('<img src="images/unknownload.gif" />');
			ajaxcheck('RS');
			ajaxcheck('HF');
			ajaxcheck('ULD');
			ajaxcheck('ULNET');
			...
			$("[id^='unknown"+type+"']").html('<img src="images/unknownload.gif" />');
			ajaxcheck(type);
		}
	}
}
function ajaxcheck(type){
	$.ajax({
		...
		success: function(resp) {
			$("#table"+type).html($("#table"+type , resp).html());
		},error:function (){
			ajaxcheck(type);
		}
	});
...
function donate(obj){
	if($("#accounts").val().length > 8) {
		$("#wait").html(get_loading);
		$.ajax({
			type: "POST",
	...
	if ($('#showlistlink').css('display') == "none"){
		...
		if(navigator.appName === "Microsoft Internet Explorer"){
			...
			$.each(linkgens,function(i) {
				if(/kick here to download/.test(linkgens[i]) === true){
...
function bbcode(type) {
	...
	if(navigator.appName === "Microsoft Internet Explorer"){
		...
		$.each(linkgens,function(i) {
			if(/File too big/.test(linkgens[i]) === true) {
				...
				var linkgen = text.exec(linkgens[i]);
				showlinkgen = showlinkgen+"[b][url=http://www.google.com/search?q="+linkgen[1]+"][color=red]Link sex :ohm ==&#9658; [/color][color=blue]"+linkgen[1]+"[/color][/b][/url][br]";
			}
			...
			else if(/kick here to download/.test(linkgens[i]) === true){
		...
		$.each(linkgens,function(i) {
			if(/File too big/.test(linkgens[i]) === true) {
				...
				var text = /href=\"(.*?)\"/g;var linkgen=text.exec(linkgens[i]);
				showlinkgen = showlinkgen+"[b][url="+linkgen[1]+"][color=blue][s]"+linkgen[1]+"[/s][/color][/url] errorlink [color=red] ==&#9658; Link Dead !!![/color][/b][br]";
			}
			else if(/kick here to download/.test(linkgens[i]) === true){
				var text = /href=\"(.*?)\" (.*?)> <font color=\"(.*?)\">(.*?)<\/font> <font color=\"(.*?)\">(.*?)<\/font>/g;
	...
	if(type === "list") {
		...
		if ($('#bbcode').css('display') == "none") $('#bbcode').show();
...
$("#autopcbox").live('click',function(){
	if($(this).is(":checked")) {
	//	alert("Cbox Autopost Activated use your user/pass");
		$("#formauto").show(500);
	}
	else {
		$("#formauto").hide(500);
