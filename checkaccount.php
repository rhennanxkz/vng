...
if (isset($_POST["check"])) {
	$check = false;
	#======================= begin check acc rapidshare.com =======================#
	if($_POST["check"]== "RS") {
			...
			for($i = 0; $i < count($obj->acc["rapidshare.com"]["accounts"]); $i++) {
				...
				//Validity				
					...
					if ($matches[1]==0) {	
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
						<td id="unknownRS"><font color=#666666><b>FREE ACC</b></font></td>
						<td id="unknownRS"><font color=green><b>Removed</b></font></td></tr>';
						$delacc[] = $i;
					} 
					else { 
			...
			for($i = 0; $i < count($obj->acc["hotfile.com"]["accounts"]); $i++) {
				...
				if(strpos($data,'Location: http://hotfile.com/')) { 
					...
					if(strpos($data,'<div class="centerSide"><p><span>Free</span>')) {
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
						<td id="unknownHF"><font color=#666666><b>FREE ACC</b></font></td>
						<td id="unknownHF"><font color=green><b>Removed</b></font></td></tr>';
						$delacc[] = $i;
					}
		...
		if(count($obj->acc["uploading.com"]["accounts"])>0) {
			...
			for($i = 0; $i < count($obj->acc["uploading.com"]["accounts"]); $i++) {
				...
				if (stristr($account,':')) {
					...
					$account = substr($account, 0, 5).'****';
					$type = "account";
					$data = $obj->curl("http://uploading.com/general/login_form/?ajax", "", "email=$user&password=$pass");
				...
				if(strpos($data,'Log in')) {
					...
					//Validity	
					if(strpos($data,"Basic")) {
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
						<td id="unknownULD"><font color=#666666><b>FREE ACC</b></font></td>
						<td id="unknownULD"><font color=green><b>Removed</b></font></td></tr>';
						$delacc[] = $i;
					}
					elseif(strpos($data,"Lifetime membership")) {
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
						<td id="unknownULD"><font color=red><b>Lifetime membership</b></font></td>
						<td id="unknownULD"><font color=blue><b>Working</b></font></td></tr>';
					}
					elseif(preg_match('/(.+ [0-9]{1,2}, [0-9]{1,4})/', $data, $matches)) {
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
						<td id="unknownULD"><font color=red><b>'.$matches[1].'</b></font></td>
						<td id="unknownULD"><font color=blue><b>Working</b></font></td></tr>';
					}
					else {
						echo '<tr class="flistmouseoff" align="center">
		...
		if(count($obj->acc["rapidgator.net"]["accounts"])>0) {
			...
			for($i = 0; $i < count($obj->acc["rapidgator.net"]["accounts"]); $i++) {
				...
				if(strpos($data,'>Registration</a>')) { 
					...
					elseif(preg_match('%Premium till (.*)\s+%U', $data, $matches)) {
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
						<td id="unknownRGT"><font color=red><b>'.$matches[1].'</b></font></td>
		...
		if(count($obj->acc["depositfiles.com"]["accounts"])>0){
			...
			for($i = 0; $i < count($obj->acc["depositfiles.com"]["accounts"]); $i++){
				...
				if (stristr($account,':')) {
					...
					elseif(strpos($data,"CaptchaRequired")) {
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
			...
			echo "</table>";		
			if(isset($delacc)) {
				foreach ($delacc as $i) unset($obj->acc["depositfiles.com"]["accounts"][$i]);
				unset($delacc);
				$check = true;
			}
		}
	...
	################################################################################
	#======================== begin check acc ryushare.com ========================#
	elseif($_POST["check"]== "RY"){
		if(count($obj->acc["ryushare.com"]["accounts"])>0){
			echo '<table id="tableRY" class="filelist" align="left" cellpadding="3" cellspacing="1" width="100%">
				<tr class="flisttblhdr" valign="bottom">
					<td width="30%"><b>accounts ryushare.com</b></td>
					<td width="15%"><b>Type</b></td>
					<td><b>Validity</b></td>
					<td width="20%"><b>Report</b></td>
				</tr>
			';
			for($i = 0; $i < count($obj->acc["ryushare.com"]["accounts"]); $i++){
				$account = $obj->acc["ryushare.com"]["accounts"][$i];
				if (stristr($account,':')) {
					list($user, $pass) = explode(':', $account);
					$account = substr($account, 0, 5).'****';
					$type = "account";
					...
					if(strpos($data,"Incorrect Login or Password")) {
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
						<td id="unknownRY"><font color=#CCFF00><b>Login failed !!!</b></font></td>
						<td id="unknownRY"><font color=green><B>Removed</B></font></td></tr>';
						$delacc[] = $i;
						continue;
					}
					else $cookie = $obj->GetCookies($data);
				...
				if(strpos($data,'Forgot password')) { 
					echo '<tr class="flistmouseoff" align="center">
					<td><b>'.$account.'</b></td><td>'.$type.'</td>
					<td id="unknownRY"><font color=#CCFF00><b>Login failed !!!</b></font></td>
					<td id="unknownRY"><font color=green><b>Removed</b></font></td></tr>';
					...
					if(strpos($data,'Upgrade to premium')) {
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
						<td id="unknownRY"><b><font color=#666666>FREE ACC</font></b></td>
						<td id="unknownRY"><font color=green><b>Removed</b></font></b></td></tr>';
						$delacc[] = $i;
					}
					elseif(preg_match('%<b>([0-9]+.+)</b>%U', $data, $matches)) {
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
						<td id="unknownRY"><font color=red><b>'.$matches[1].'</b></font></td>
						<td id="unknownRY"><font color=blue><b>Working</b></font></td></tr>';
					}
					else{
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
						<td id="unknownRY">unknown</td>
						<td id="unknownRY">unknown</td></tr>';
					}
				}
			}
			echo "</table>";
			if(isset($delacc)) {
				foreach ($delacc as $i) unset($obj->acc["ryushare.com"]["accounts"][$i]);
				unset($delacc);
				$check = true;
			}
		}
	}
	#======================== end check acc ryushare.com ==========================#
	################################################################################
		...
		if(count($obj->acc["real-debrid.com"]["accounts"])>0) {
			...
			for($i = 0; $i < count($obj->acc["real-debrid.com"]["accounts"]); $i++) {
				...
				if(strpos($data,'<a href="#login-box" rel="facebox">Login</a>')) { 
					...
					//Validity	
					if(strpos($data,'<span class="fidelity">Free</span>')) {
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
					...
					elseif(preg_match('%<li>Premium :<span class="fidelity">(.*)</span></li>%U', $data, $matches)) {
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
			...
			for($i = 0; $i < count($obj->acc["alldebrid.com"]["accounts"]); $i++) {
				...
				if(strpos($data,'Location')) {
					...
					elseif(strpos($data,'</strong>Premium</li>')) {
						preg_match("%You have now <strong>(.*)</strong> %U", $data, $matches);
						echo '<tr class="flistmouseoff" align="center">
						<td><b>'.$account.'</b></td><td>'.$type.'</td>
	...
	if(count($obj->acc["rapidgator.net"]["accounts"])>0) {
		...
		for($i = 0; $i < count($obj->acc["rapidgator.net"]["accounts"]); $i++) {
			...
			$account = substr($account, 0, 5)."****";
			echo '<tr class="flistmouseoff" align="center"><td><b>'.$account.'</b></td><td>'.$type.'</td><td id="unknownRGT">unknown</td><td id="unknownRGT">unknown</td></tr>';
		}
		echo "</table>";
	...
	if(count($obj->acc["ryushare.com"]["accounts"])>0){
		...
		for($i = 0; $i < count($obj->acc["ryushare.com"]["accounts"]); $i++){
			...
			$account = substr($account, 0, 5)."****";
			echo '<tr class="flistmouseoff" align="center"><td><b>'.$account.'</b></td><td>'.$type.'</td><td id="unknownRY">unknown</td><td id="unknownRY">unknown</td></tr>';
		}
		echo "</table>";
		echo "<a onclick=\"checkacc('RY');\" href=\"javascript:void(0)\" style='TEXT-DECORATION: none'><font color=#FF6600>Check accounts ryushare.com >>></font></a><br><br>";
		$checkall = true;
	}
	if(count($obj->acc["real-debrid.com"]["accounts"])>0) {
		echo '<table id="tableREAL" class="filelist" align="left" cellpadding="3" cellspacing="1" width="100%">
